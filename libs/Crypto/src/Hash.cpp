...
static const charCountSet countChars(const char * str) {
        charCountSet ret = { 0, 0, 0 };
...
{
        int len = data.length();
        const charCountSet count = countChars(data.data());
        double spec_c = static_cast<double>(count.specials)/len,
               nums_c = static_cast<double>(count.digits)/len,
               alph_c = static_cast<double>(count.alphas)/len;

        // ПОД КАКАИМИ ВЕЩЕСТВАМИ ЭТО БЫЛО НАПИСАНО?
...
passwordQuality(const QByteArray& password)
{
        ...
        const charCountSet count = countChars(password.data());
